...
function ModalDetailHistory({ isOpen, transaction, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-500 bg-opacity-50">
            <div className="p-6 text-gray-500 bg-white rounded-lg dark:text-white dark:bg-gray-800 w-1/2">
                <div className='flex justify-center'>
                    <h3 className="mb-6 text-2xl font-bold text-gray-900 dark:text-gray-200">Detail Riwayat Pembelian</h3>
                </div>
                <div>
                    <p className="font-semibold">Tanggal Transaksi: <span className='font-medium text-gray-500 dark:text-gray-400'>{new Date(transaction.tanggal).toLocaleString()}</span></p>
                    <p className="font-semibold">Total: <span className='font-medium text-gray-500 dark:text-gray-400'> Rp. {transaction.total}</span></p>
                </div>
                <div className="mt-4">
