...
function CommonToast({ type = 'success', isOpen, message, onClose }) {
    ...
    const { iconBg, iconColor, iconPath } = iconStyles[type] || iconStyles.success;
    ...
    return (
        ...
        >
            <div
                className="flex items-center w-full p-4 text-gray-500 font-medium bg-white rounded-lg shadow-lg shadow-blue-100 dark:text-gray-400 dark:bg-gray-800"
                role="alert"
            >
                <div
                    className={`inline-flex items-center justify-center shrink-0 w-8 h-8 rounded-lg ${iconBg} ${iconColor}`}
                >
                    <svg
                        className="w-5 h-5"
                        aria-hidden="true"
                    ...
                    <span className="sr-only">
                        {type === 'success' ? 'Success' : 'Error'} icon
                    </span>
                </div>
